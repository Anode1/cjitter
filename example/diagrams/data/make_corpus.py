"""Write the text corpora station reads, from the parsers' JSON in ../pilot/pr_*/.
    ...
    python3 make_corpus.py ../pilot/pr_hs hs.txt [--neato hs_neato.txt] [--exclude WP5037 WP3391]
...
station rescales. With --neato, the same graphs laid out by neato
(graphviz) with the same box sizes, start seeded so the file reproduces, as the tool control.
"""
...
def neato(g):
    ...
    dot.append("}")
    o = subprocess.run(['neato', '-Gstart=1', '-Tplain'], input="\n".join(dot),
...
if __name__ == '__main__':
    ...
    neato_out = None; exclude = []
    args = sys.argv[3:]
    while args:
        if args[0] == '--neato': neato_out = args[1]; args = args[2:]
    ...
    if neato_out:
        n = 0
        with open(neato_out, 'w') as f:
            f.write("# the graphs of %s laid out by neato -Gstart=1, same box sizes\n" % dst)
            for g in G:
                xy = neato(g)
                if xy is None: continue
                write(f, g, xy); n += 1
        print("%s: %d graphs" % (neato_out, n))
