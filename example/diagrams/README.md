...
## Files
    ...
    results/*.md         the review-driven measurements, each generated by the script
                         beside it: robustness (directions, HOLA clustering, boundary
                         pileups, Kendall's W), constrained (stress subject to alignment),
                         dedup, drawers (WikiPathways authorship), border (box-aware
                         distances), gdcoll (the GD Collection through the test), era
                         (the hold by tool generation), tolsweep, specgrid (1,008 cells),
                         calib (the displacement yardstick), bpmn_random300 and
                         bpmn_population (the selection bias confined to A1), band41
                         (the registered 41-100 band)
