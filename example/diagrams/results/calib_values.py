...
def medE(path):
    ...
    return st.median(float(r['E']) for r in csv.DictReader(io.StringIO(o)))
tmp = tempfile.mkdtemp(prefix='calib_values_')
EPS = [0.005, 0.01, 0.02, 0.05, 0.10]
print('| corpus | neato | ' + ' | '.join('eps %g' % e for e in EPS) + ' | hand | random |\n|' + '---|' * (len(EPS) + 4))
for ci, c in enumerate(['hs', 'sbgn', 'bpmnr']):
    row = ['%.3f' % medE(os.path.join(DATA, c + '_neato.txt'))]
    for ei, eps in enumerate(EPS):
        dst = os.path.join(tmp, '%s_e%g.txt' % (c, eps)); perturb(os.path.join(DATA, c + '_neato.txt'), dst, eps, 100 * ci + ei)
        row.append('%.3f' % medE(dst))
    row.append('%.3f' % medE(os.path.join(DATA, c + '.txt')))
    rnd = os.path.join(tmp, c + '_random.txt'); nn['write_null'](os.path.join(DATA, c + '_chords.txt'), rnd, 'random')
    row.append('%.3f' % medE(rnd))
    print('| %s | %s |' % (c, ' | '.join(row)))
