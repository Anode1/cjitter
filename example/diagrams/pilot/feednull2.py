...
for fn in sorted(glob.glob(os.path.join(d,'*.json'))):
    ...
    o=subprocess.run(['./stat5',str(budget),'12345',mode,cap],
...
MH=[r[8] for r in rows]; MS=[r[9] for r in rows]
print("  displacement from the HUMAN layout     : median %.1f%% of the cap"%(100*st.median(MH)))
print("  displacement from a CONVERGED OPTIMUM  : median %.1f%% of the cap   <- the null"%(100*st.median(MS)))
