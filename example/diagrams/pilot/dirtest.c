...
static double climb(double*x,long budget,long*used){
    double f=energy(x),jit=JIT0; long ev=1,rej=0,pat=40+10*N; int k=0;
    ...
    while(ev<budget){
        ...
        else { if(++rej>=pat){ jit*=0.5; rej=0; if(jit<1e-6) jit=JIT0; } }
    }
    free(y); *used=ev; return f;
}
/* Directional test. For every node, D directions at radius d; a node is "held" if no
 * direction lowers the energy. Prints per graph: N, q = fraction of nodes held, and the
 * mean best decrease per node as a fraction of E. Mode h: human layout. Mode c: layout
 * first descended to convergence (uncapped, budget evals), then tested. */
static int Ns[4096],Ms[4096]; static double *Xs[4096],*Ws[4096],*Hs[4096];
static int *EAs[4096],*EBs[4096]; static int K;
int main(int argc,char**argv)
{
    char mode=argv[1][0]; long budget=atol(argv[2]); unsigned seed=(unsigned)atoi(argv[3]);
    double cap=atof(argv[4]); int D=atoi(argv[5]);
    Wc=atof(argv[6]); Wl=atof(argv[7]); Wo=atof(argv[8]);
    if(argc>9) Wr=atof(argv[9]); if(argc>10) Wa=atof(argv[10]); if(argc>11) Wn=atof(argv[11]);
    if(scanf("%d",&K)!=1) return 1;
    for(int g=0; g<K; g++){
        int n,m; if(scanf("%d %d",&n,&m)!=2) return 1;
        Ns[g]=n; Ms[g]=m;
        Xs[g]=malloc(2*n*sizeof(double)); Ws[g]=malloc(n*sizeof(double)); Hs[g]=malloc(n*sizeof(double));
        EAs[g]=malloc(m*sizeof(int)); EBs[g]=malloc(m*sizeof(int));
        for(int i=0;i<n;i++) if(scanf("%lf %lf %lf %lf",&Xs[g][2*i],&Xs[g][2*i+1],&Ws[g][i],&Hs[g][i])!=4) return 1;
    ...
    for(int g=0; g<K; g++){
        ...
        for(int i=0;i<M;i++){ double dx=Xs[g][2*EA[i]]-Xs[g][2*EB[i]], dy=Xs[g][2*EA[i]+1]-Xs[g][2*EB[i]+1]; d[i]=sqrt(dx*dx+dy*dy); }
        for(int i=0;i<M;i++) for(int j=i+1;j<M;j++) if(d[j]<d[i]){double t=d[i];d[i]=d[j];d[j]=t;}
        L = d[M/2]>0 ? d[M/2] : 1.0/sqrt((double)N); free(d);
        double *x=malloc(2*N*sizeof*x); memcpy(x,Xs[g],2*N*sizeof*x);
        if(mode=='c'){ long used; CAP=1e9; X0=NULL; climb(x,budget,&used); }
        double e=energy(x), *y=malloc(2*N*sizeof*y); int held=0; double dec=0;
        for(int i=0;i<N;i++){
            double best=0;
            for(int k=0;k<D;k++){
                double a=6.283185307179586*k/D;
                memcpy(y,x,2*N*sizeof*y);
                y[2*i]+=cap*cos(a); y[2*i+1]+=cap*sin(a);
                double f=energy(y); if(e-f>best) best=e-f;
            }
            if(best<=1e-12) held++; else dec+=best;
        }
        printf("%d %.6f %.6f\n", N, (double)held/N, e>0? dec/N/e : 0);
        free(x); free(y);
    }
    return 0;
}
