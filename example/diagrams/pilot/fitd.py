...
def displacement(graphs, wc, wl, wo):
    inp = "%d\n" % len(graphs) + "\n".join(graphs)
    o = subprocess.run(['./stat4', BUDGET, '12345', CAP, str(wc), str(wl), str(wo)],
    ...
    for line in o.stdout.strip().split("\n"):
        ...
        if e > 0: r.append(100.0*float(v[6]))
    return st.median(r) if r else 100.0

base = displacement(test, 1, 1, 1)
print("weights (1,1,1), asserted        : held-out displacement %.1f%% of the cap" % base)
best = None
for i in range(DRAWS):
    wc = 10**random.uniform(-2, 2); wl = 10**random.uniform(-2, 2); wo = 10**random.uniform(-2, 2)
    s = wc+wl+wo; wc, wl, wo = 3*wc/s, 3*wl/s, 3*wo/s
    r = displacement(train, wc, wl, wo)
    if best is None or r < best[0]: best = (r, wc, wl, wo)
print("best on TRAIN: displacement %.1f%% of cap, weights cross %.3f length %.3f overlap %.3f"
      % best)
held = displacement(test, best[1], best[2], best[3])
print("same weights on HELD-OUT half    : displacement %.2f%%" % held)
print("so fitting the weights moves the displacement %.2f -> %.2f points" % (base, held))
