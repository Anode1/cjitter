...
static double energy(const double*x){
    ...
    for(i=0;i<N;i++){ anode+=W[i]*H[i];
        ...
        for(j=0;j<N;j++){
            ...
            double mn=ax2<ay2?ax2:ay2;      /* lined up on a row or a column */
            if(mn<bx) bx=mn;
        }
        if(bx<1e8) align+=bx;
        for(j=i+1;j<N;j++){
...
int main(int argc,char**argv)
{
    ...
    for(int g=0; g<K; g++){
        ...
        mv/=(double)N*cap;
        double eend=energy(h);           /* recompute to expose the terms after descent */
        printf("%d %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n",
               N, e_h, e_hd, sc, sl, so, mv, TO, TC, TA);
        (void)eend;
